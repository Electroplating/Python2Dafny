...
settings.max_fixing_iterations = 5
...
def generate(
    ...
    # ]
    code = "\n".join(code)
    DafnyFilePath = Path(settings.translation_path).joinpath(task_id + ".dfy")
...
def solve(
    ...
    status = "translate"
    code = problems[task_id]["SourceCode"]
    error_messages = []
    for iter in range(settings.max_fixing_iterations):
        generate(task_id, code, status, error_messages)
        old_status = copy.deepcopy(status)
        old_error_messages = copy.deepcopy(error_messages)
        status, error_messages = realtime_eval(
            Path(settings.translation_path).joinpath(task_id + ".dfy"),
...
for task_id in problems:
    Path(settings.translation_path).mkdir(parents=True, exist_ok=True)
    thread = Thread(
        target=solve,
        args=(task_id,),
    )
